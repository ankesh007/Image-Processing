...
for i in range(args.k):
    ...
    for i in range(total_feature_points):
        interim_r=int(src_points[i][0]*lef_wt+dest_points[i][0]*rig_wt)
        interim_c=int(src_points[i][1]*lef_wt+dest_points[i][1]*rig_wt)
    ...
    # point_index=point_index[:,np.newaxis]
    # print(triangulation.transform[point_index,:2].shape)
    # print(point_index.shape)
    # print(all_points.shape)
    # print(triangulation.transform[point_index,2].shape)
    b = triangulation.transform[point_index,2].dot(all_points - triangulation.transform[point_index,2])
    baricentric_coord=np.c_[b, 1 - b.sum(axis=1)]
    print(baricentric_coord.shape)
    print(baricentric_coord)
