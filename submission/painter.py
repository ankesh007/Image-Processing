...
def getSumRectangle(dp, i1, j1, k1, i2, j2, k2):
    return dp[i2][j2][k2] - (dp[i1 - 1][j2][k2] + dp[i2][j1 - 1][k2] + dp[i2][j2][k1 - 1]) + (
        dp[i1 - 1][j1 - 1][k2] + dp[i2][j1 - 1][k1 - 1] + dp[i1 - 1][j2][k1 - 1]) - dp[i1 - 1][j1 - 1][k1 - 1]
...
def median_cut_quantization(img_inp, keep_colors=10, dither=False):
    ...
    for i in range(1, color_levels + 1, 1):
        for j in range(1, color_levels + 1, 1):
            for k in range(1, color_levels + 1, 1):
                dp[i][j][k] = accu[i][j][k] + dp[i - 1][j][k] + dp[i][j - 1][k] + dp[i][j][k - 1] + dp[i - 1][j - 1][
                    k - 1] - (dp[i - 1][j - 1][k] + dp[i][j - 1][k - 1] + dp[i - 1][j][k - 1])
    ...
    while (boxes < keep_colors):
        ...
        # print(box_coord)
        cut_val = getMedian(dp, one_box[0][0], one_box[1][0], one_box[2][0], one_box[0][1], one_box[1][1],
                            one_box[2][1], axis)
...
quant = (median_cut_quantization(img, color_palette, dither))

img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
cv2.imshow('src', img)
xdog = XDoG(img, sigma, k, tau, eps, phi)
...
for i in range(h):
    for j in range(w):
        paint[i][j] = xdog[i][j] * quant[i][j]

cv2.imshow('painted', paint)
cv2.imwrite(sys.argv[2], paint)
